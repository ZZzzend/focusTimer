 ...
 //   let userDefaults = UserDefaults.standard
    ...
    func workAndPauseState() {
        ...
        case .pauseTimer(let secondsRemaind, let previousState):
            ...
            case .workTimerIsActive:
            //    timerAction.timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(timerAction.unpauseCounter), userInfo: Date(), repeats: true)
                timerAction.timerSheduled(select: #selector(timerAction.unpauseCounter))

            case .breakTimerIsActive:
           //     timerAction.timer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(timerAction.breakUnpauseCounter), userInfo: Date(), repeats: true)
